  ...
  return (
    <div className="fixed left-0 top-0 w-full h-screen  overflow-y-auto">
      <div className="w-full sm:w-4/6 bg-white absolute top-0 left-0 h-screen">
        ...
        <div className="absolute left-0 top-[64px] bg-gray-100 h-[calc(100%-64px)] w-full p-8 overflow-hidden overflow-y-auto">

          <div className="flex justify-start items-center overflow-hidden overflow-x-auto py-4 px-8">
            <span className="px-6 text-sm cursor-pointer active:bg-green-700 active:text-white select-none font-bold rounded py-1 mx-2 bg-white">All</span>
          </div>

          <div className="flex justify-center items-start flex-wrap overflow-hidden overflow-y-auto">
            {Array(200).fill().map((_, i) => (
              <ProductCart key={i} onClick={() => addProduct(i)} />
            ))}
          </div>
        </div>
      </div>
